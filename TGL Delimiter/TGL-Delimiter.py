...
class TGLDelimiter(tk.Tk):
    ...
    @staticmethod
    def generate_traps_in_square(num_traps, lambdas):
        # Define the dimensions of the grid (1 square mile)
        grid_size = 1609  # 1 mile = 1609 meters
        ...
        x_coordinates = np.linspace(0, grid_size, traps_per_axis, endpoint=False)
        y_coordinates = np.linspace(0, grid_size, traps_per_axis, endpoint=False)
        ...
        for sides in range(4):
            if sides == 0:
                for squares in range(1, length_ + 1):
                    for i in range(len(list_)):
                        x, y, lambdas = list_[i]
                        x += (start_ + squares - 1) * 1609 + spacing
                        y += start_ * 1609 + spacing
            ...
            if sides == 1:
                for squares in range(1, length_ + 1):
                    for i in range(len(list_)):
                        x, y, lambdas = list_[i]
                        x += (start_ + length_ - 1) * 1609 + spacing
                        y += (start_ + squares - 1) * 1609 + spacing
                        if (x, y, lambdas) not in new_traps:
                            new_traps.append((x, y, lambdas))
            if sides == 2:
                for squares in range(1, length_ + 1):
                    for i in range(len(list_)):
                        x, y, lambdas = list_[i]
                        x += (start_ + squares - 1) * 1609 + spacing
                        y += (start_ + length_ - 1) * 1609 + spacing
                        if (x, y, lambdas) not in new_traps:
                            new_traps.append((x, y, lambdas))
            else:
                for squares in range(1, length_ + 1):
    ...
    @staticmethod
    def add_core_area(length_, list_):
        new_traps = []
        sorted_traps = sorted(list_, key=lambda trap: trap[0])
        lowest_x_value = sorted_traps[0][0]
        for next_lowest in sorted_traps[1:]:
        ...
        for i in list_:
            x, y, lam = i
            x += (length_ - 1) * 1609 + spacing
            y += (length_ - 1) * 1609 + spacing
    ...
    def main(self):
        ...
        squares_list = TGDelimiter.generate_squares_by_densities(self.density_list, self.lambdas)
        grid = []
